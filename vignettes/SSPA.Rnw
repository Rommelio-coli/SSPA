...
\date{Modified: 8 May, 2013. Compiled: \today}
...
<<simulated-data-plotting, include=false, fig=TRUE>>=
...
\begin{figure}[!tbp]
  \centering
  \includegraphics[width=0.6\textwidth]{SSPA-simulated-data-plotting}
  \caption{Diagnostic plots of test statistics and p-values of the pilot-data: Upper-left histogram of test statistics, Upper-right histogram of p-values, Lower-left qq-plot of test statistics and Lower-right p-values in increasing order.}
\end{figure}
...
<<simulated-data-deconvolution, include=false, fig=TRUE>>=
...
\begin{figure}[!tbp]
  \centering
  \includegraphics[width=0.6\textwidth]{SSPA-simulated-data-deconvolution}
  \caption{Estimated density and true (bitriangular) density of effect sizes using deconvolution.}
\end{figure}
...
<<simulated-data-power, include=false, fig=TRUE>>=
...
\begin{figure}[!tbp]
  \centering
  \includegraphics[width=0.6\textwidth]{SSPA-simulated-data-power}
  \caption{Predicted power for sample sizes larger than the pilot-data.}
\end{figure}

The density of effect sizes can also be estimated using a conjugate gradient algorithm \Rcode{method='congrad'} or using Tikhonov regularization \Rcode{method='tikhonov'} see figures \ref{simulated-data-effectsize} and \ref{tikhonov} for the results on the simulated data in the Appendix.  
...
<<nutrigenomics-effectsize, include=false, fig=TRUE>>=
library(lattice)
data(Nutrigenomics)
str(Nutrigenomics)
...
\begin{figure}[!tbp]
  \centering
  \includegraphics[width=0.6\textwidth]{SSPA-nutrigenomics-effectsize}
  \caption{Estimated densities of effect sizes for the nutrigenomics experiment.}
\end{figure}

<<nutrigenomics-power, include=false, fig=TRUE>>=
samplesize <- seq(2, 8)
...
\begin{figure}[!tbp]
  \centering
  \includegraphics[width=0.6\textwidth]{SSPA-nutrigenomics-power}
  \caption{Predicted power curves for the nutrigenomics experiment.}
\end{figure}
...
<<deepsage-effectsize, include=false, fig=TRUE>>=
library(lattice)
data(deepSAGE)
str(deepSAGE)
...
\begin{figure}[!tbp]
  \centering
  \includegraphics[width=0.6\textwidth]{SSPA-deepsage-effectsize}
  \caption{Estimated density of effect sizes and predicted power curve for the deepSAGE experiment.}
\end{figure}
...
<<simulated-data-effectsize, include=false, fig=TRUE>>=
...
\begin{figure}[!tbp]
  \centering
  \includegraphics[width=0.6\textwidth]{SSPA-simulated-data-deconvolution}
  \caption{Estimated density and true (bitriangular) density of effect sizes using a conjugate gradient algorithm.}
  \label{simulated-data-effectsize}
\end{figure}

\section{Tikhonov regularization}

<<tikhonov, include=false, fig=TRUE>>=
ssT <- sampleSize(pdC, 
                  method="tikhonov", 
...
\begin{figure}[!tbp]
  \centering
  \includegraphics[width=0.6\textwidth]{SSPA-tikhonov}
  \caption{Estimated density and true (bitriangular) density of effect sizes using Tikhonov regularization.}
  \label{tikhonov}
\end{figure}
